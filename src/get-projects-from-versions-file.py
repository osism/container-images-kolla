...
if BUILD_TYPE in ["all", "base"]:
    ...
    all_projects = [x for x in versions["openstack_projects"] if x in OPENSTACK_CORE_PROJECTS]
elif BUILD_TYPE == "openstack-additional":
    all_projects = [x for x in versions["openstack_projects"] if x not in OPENSTACK_CORE_PROJECTS]
