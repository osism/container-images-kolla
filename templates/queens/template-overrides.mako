...
{% block skydive_agent_header %}
# NOTE(berendt): Upstream patch available, can be dropped in Rocky (https://review.openstack.org/#/c/548608/).
RUN apt-get update ${"\\"}
    && apt-get -y install --no-install-recommends openvswitch-common ${"\\"}
    && apt-get clean ${"\\"}
    && rm -rf /var/lib/apt/lists/*
{% endblock %}
...
% for project in ("openstack", "openvswitch", "redis", "skydive", "aodh", "ceilometer", "cinder", "cloudkitty", "designate", "glance", "gnocchi", "heat", "keystone", "magnum", "manila", "mistral", "neutron", "nova", "novajoin", "octavia", "panko", "watcher", "barbican"):
