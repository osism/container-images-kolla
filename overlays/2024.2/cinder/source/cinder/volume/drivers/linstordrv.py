...
linstor_opts = [
    ...
    cfg.StrOpt(
        "linstor_default_storage_pool_name", help="Default LINSTOR Storage Pool to use."
    ),
    cfg.StrOpt(
    ...
    cfg.BoolOpt(
        "linstor_force_udev",
        default=True,
        help="True, if the driver should assume udev created links" "always exist.",
    ),
    cfg.BoolOpt(
        "linstor_use_snapshot_based_clone",
        default=False,
    ...
    * A Cinder Volume Type maps to a Resource Group in Linstor.
    ...
    def check_for_setup_error(self):
        ...
        if not hasattr(linstor, "MultiLinstor"):
            msg = _(
                "Package python-linstor does not support MultiLinstor, " "please update"
            )
            raise LinstorDriverException(msg)
        ...
        try:
            ...
            msg = _(
                "Package python-linstor does not support passing clients "
        ...
        if rest_version < (1, 4, 0):
            msg = _("Linstor API not supported: %s < (1, 4, 0)") % str(rest_version)
            raise LinstorDriverException(msg)

        if len(nodes.nodes) < 1:
            msg = (
                _("Cinder host %s is not a configured Linstor " "node") % self._hostname
            )
            raise LinstorDriverException(msg)
        ...
        if self._use_direct_connection():
            ...
            target_driver = self.target_mapping[self.configuration.target_helper]

            LOG.debug(
                "Attempting to initialize LINSTOR driver with the "
                "following target_driver: %s",
                target_driver,
            )

            self.target_driver = importutils.import_object(
                target_driver, configuration=self.configuration, executor=self._execute
            )  # type: targets.Target
            self.protocol = self.target_driver.protocol
    ...
    def _init_vendor_properties(self):
        ...
        self._set_property(
            ...
            type="str",
            default=self.configuration.safe_get("linstor_default_storage_pool_name"),
        )
        self._set_property(
            self._vendor_properties,
    ...
    def _resource_group_for_volume_type(self, volume_type):
        ...
        layer_list = self._get_linstor_property("layer_list", volume_type)
        ...
        on_same = self._get_linstor_property("replicas_on_same", volume_type)
    ...
    def create_volume_from_snapshot(self, volume, snapshot):
        ...
        try:
            ...
            LOG.exception(
                "Could not resize restored Linstor volume, " "deleting volume"
            )
            rsc.delete()
            raise
    ...
    def revert_to_snapshot(self, context, volume, snapshot):
        ...
        try:
            ...
            LOG.info(
                "Failed to rollback snapshot, retrying with v1 driver " "name %s",
                "SN_" + snapshot["id"],
            )
    ...
    def _update_volume_stats(self):
        ...
        tot = _kib_to_gib(sum(p.free_space.total_capacity for p in storage_pools))
        free = _kib_to_gib(sum(p.free_space.free_capacity for p in storage_pools))
        provisioned_cap = _kib_to_gib(
            sum(vd.size for rd in resource_dfns for vd in rd.volume_definitions)
        )
    ...
    def retype(self, context, volume, new_type, diff, host):
        ...
        LOG.debug(
            "LINSTOR retype called for volume %s. No action "
    ...
    def migrate_volume(self, context, volume, host):
        ...
        if self._linstor_uri_str != target_ctrl:
            LOG.debug(
                "Target is not using the same controllers: %s != %s",
        ...
        if (
            volume["status"] in {"attached", "in-use"}
            and self.protocol != target_protocol
        ):
            LOG.debug(
                "Cannot migrate attached volume between different "
    ...
    def ensure_export(self, context, volume):
        ...
        if not self._use_direct_connection():
            LOG.debug(
                "using non-direct driver method, need to create local "
    ...
    def remove_export(self, context, volume):
        ...
        if not self._use_direct_connection():
            LOG.debug(
                "using non-direct driver method, need to delete local "
                "replica on cinder host"
            )
            rsc = _get_existing_resource(
                self.c.get(),
        ...
        if volume["multiattach"]:
            if (
                sum(
                    ...
                    if a.connector
                    and a.connector["initiator"] == connector["initiator"]
                )
                > 1
            ):
                return True
...
class LinstorDirectTarget(targets.Target):
    ...
    def initialize_connection(self, volume, connector):
        ...
        if connector["host"] not in _attached_on(volume):
            LOG.debug(
                "Trying to attach to a volume in use, looks like live "
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        ...
        if connector is None:
            LOG.debug(
                "force detach of volume, no clever deactivating of "
                "resources required"
            )
        ...
        if volume["status"] == "in-use" and any(
            x != connector["host"] for x in _attached_on(volume)
        ):
            LOG.debug(
                "Trying to detach from a volume in use, looks like live "
...
def _get_existing_resource(linstor_client, volume_name, volume_id):
    ...
    """
    LOG.debug("Searching existing volume %s in LINSTOR " "backend", volume_name)
    ...
    msg = (
        _("Found no matching resource in LINSTOR backend for " "volume %s")
        % volume_name
    )
    raise LinstorDriverException(msg)
...
def _find_symlink_to_device(linstor_client, resource_name, node):
    ...
    links = [
        v
        for k, v in volume.properties.items()
...
def _attached_on(volume):
    ...
    :rtype: list[str]
    """
    return [attachment["attached_host"] for attachment in volume["volume_attachment"]]
